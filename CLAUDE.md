...
| i3 | Audio-ID | BirdNET Select-TF-ops på iOS, AVAudioEngine-capture | ⬜ |
| i4 | Parity sweep | Karta (MapLibre iOS), notiser, PDF-export, rest | ⬜ |
...
- **`./gradlew detekt` analyserar INGEN KMP-modul** (upptäckt i i2b:s slutreview 2026-07-26, verifierad). `allprojects { apply(detekt) }` applicerar plugin:et, men `detekt { config.setFrom(...) }`-blocket ligger på **rot-projektet** och konfigurerar inte subprojekten → varje subprojekt kör med detekts defaults `src/main/{java,kotlin}`. Ingen KMP-modul har `src/main` (`composeApp`, `shared/ml`, `shared/domain`, `shared/data`, `shared/content`, `shared/datastore` = alla `commonMain|androidMain|iosMain|…`); **bara `androidApp/src/main` matchar**. Alltså: varje grind i den här filen, i plan-dokumenten och i `ci.yml` som anför "detekt grön" som bevis för delad kod har varit tom — ktlint är den enda statiska analys som faktiskt kört där. Läs INTE detta som att delad kod är ogranskad (detekt hittar kodlukt, inte buggar; koden har ktlint + tester + per-task-review + device-verify + produktion bakom sig). Fix = `detekt { source.setFrom(...) }` i `allprojects`, eller detekts per-source-set-multiplatform-tasks; räkna med en backlog vid första körningen. **Egen chore, inte en drive-by.**
