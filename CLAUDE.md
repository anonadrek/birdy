...
- **iOS-spåret (v2) — i2 brainstormad + uppdelad; i2b spec+plan KLARA, redo att köras i ny session (2026-07-12, Mac):** Research (4 agenter: ML-stack, kamera/crop, TFLite/16 KB, LiteRT-iOS) gav tre nyckelfynd: (1) iOS-ML-runtimen = **vendored `TensorFlowLiteC.xcframework`** (2.17.0, cinterop mot C-API, INGEN CocoaPods, noll ny Swift) — det finns ingen separat "LiteRT-iOS"-pod; (2) Android-LiteRT-migrationen och iOS-runtimen är **oberoende** (olika distributioner, delar bara `.tflite`-formatet) → specens "Android först"-motiv gäller inte, fri sekvensering; (3) Android-swappen `com.google.ai.edge.litert:litert:1.4.1` fixar 16 KB för FOTO-`.so` men **INTE** audio-Flex-`.so` (ingen LiteRT-select-ops-artefakt finns) → separat törne (community/self-built 16 KB Flex-lib, eller flytta FlexRFFT off-graph). **Beslut: i2 delas i tre oberoende planer** — **i2a** Android-LiteRT + 16 KB (Windows-maskinen, Galaxy-verify, vC126-ready), **i2b** iOS foto-ID (Mac), **i2c** iOS live-kamera + ta-foto = Milestone 1 (Mac, bygger på i2b:s runtime). **i2b designad + planerad:** spec `docs/superpowers/specs/2026-07-12-ios-i2b-photo-id-design.md` + plan `docs/superpowers/plans/2026-07-12-ios-i2b-photo-id.md` (7 tasks, **spike-först**: T1 = cinterop-smoke/1 inferens innan UI). i2b-beslut: cinterop + **committad** xcframework (verifierad nedladdnings-URL i specen §5 — ~76,5 MB tarball, behåll bara `TensorFlowLiteC.xcframework` CPU/XNNPACK), **galleri-only** (PHPicker, behörighetsfritt; ta-foto → i2c), crop-skärmen lyfts till **delad** commonMain (`ImageBitmap`). `IosTfliteRunner` speglar `AndroidTfliteRunner` (uint8-quant, scale 1/128 zp 128 in / 0.00390625 zp 0 ut); numerisk paritetskoll mot `tools/ml-eval`-golden. Plan-wiring-koden signatur-verifierad mot källan (`BirdClassifierFactory.create()`→`Pair`, `onCrashlytics` krävs, `tfliteSha256`, `normalizationMean` = `List<Float>` → `.toFloatArray()`). **NÄSTA SESSION (Mac): "fortsätt med i2b" → kör planen** (subagent-driven-development). i2a kan köras oberoende på Windows-maskinen närhelst. AIY-fotomodellen ligger redan i `shared:ml` compose-resources → laddas gratis på iOS.
...
| i2b | iOS foto-ID (ML-runtime + galleri-scan) | Vendored TensorFlowLiteC-cinterop-runner + ImagePreprocessor.ios + PHPicker-scan + delad crop | ⬜ **spec+plan KLARA** (2026-07-12) → nästa Mac-session kör |
