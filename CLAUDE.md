...
| i1 | Encyclopedia + journal på fysisk iPhone | SQLDelight native driver, artbilder, lätta actuals, device-install | 🔄 kod klar + agent-verifierad (prefs-persistens, native-driver save-test, bilder bundlade, bootar i sim); kvar = Albins visuella sim/iPhone-verify (WebP-dekod) + free-Apple-ID device-signing |
| i2 | LiteRT-migration + foto-scan | Android TFLite→LiteRT (16 KB-fixen!), AVFoundation-kamera, foto-ID + crop | ⬜ **Milestone 1** = live scan på iPhone; Android vC126-ready |
