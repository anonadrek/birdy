...
**Status (2026-05-14):** Plan 1 ✅ (`v0.1.0-foundation`). Plan 2a ✅ (`v0.2.0a-pipeline`). Plan 2b ⏸ pausad vid 273/700, nästa familj = cuculidae. Plan 3 ✅ (`v0.3.0-encyclopedia`). Plan 4a ✅ (`v0.4.0a-camera-ui`). Plan 4b (Real TFLite) ✅ (`v0.4.0b-real-tflite`). Plan 5a (Diary) ✅ (`v0.5.0a-diary`). Plan 5b (Gamification) ✅ (`v0.5.0b-gamification`). Plan 7a–e ✅ (foundation/screens/field-journal/match-flow/premium — alla taggade `v0.7.0a..e`). **Plan 6a (Foundation = UX-polish + release-mekanik) ⏳ IN PROGRESS — 5/15 tasks code-complete (HEAD `11a5394`, T5 device-verify pending):** T1 R8/ProGuard, T2 signing config + upload-keystore, T3 icon-koncept (användaren gjorde eget Field Journal-set i `docs/superpowers/icon-concepts/final/`), T4 adaptive launcher + Splash API 31+ + AccentCopper-token retintad `#8C5A3C` → `#A8552D`, **T5 cold-start TFLite-flytt → `ClassifierBootstrap` (sealed Initializing/Ready/Failed StateFlow) + `AppGate` state-gating + `MainActivity.onDestroy` close() för native-handle-cleanup; `JournalLoading` + `JournalDialog` stubs (T8 ersätter). Device-verify (launcher → splash → "Förbereder fältboken…" → AppScaffold) återstår tills S23 Ultra pluggas in.** Nästa = Task 6 (Locale-handling). Plan 6b (Billing + Premium-feature-leverans) följer mot `v1.0.0`.
...
| 6a | Foundation — UX-polish + release-mekanik (R8, signing, icon, splash, locale, a11y) | ⏳ 5/15 code-complete (T5 device-verify pending), mål `v0.8.0-rc1` |
