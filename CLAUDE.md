...
- **Personal finds map + polish → vC123 (2026-06-08):** `feat/map-polish-v2` (osmdroid + MapTiler private on-device finds map: opt-in location capture free, map-view Premium; merged to main as PR #4 then polished) is **merged to `main`** with three items DONE + device-verified on SM-S918B: **(1)** HiDPI `@2x` tiles + 8 download threads (crisp/fast); **(2)** Field Journal map theme — toner-v2 tiles + runtime duotone `ColorMatrix` (paper↔sepia) + wax-seal Birdy-bird pins; **(3)** photo uploads now geotag. **Map item 4 gotcha (worth remembering):** gallery + take-photo finds didn't reach the map; the EXIF design (read the photo's GPS) was built + unit-green but returned **null on device** — the **Android photo picker strips GPS EXIF** without `ACCESS_MEDIA_LOCATION` (dangerous permission). Per product call we ship **current-location for uploads** (gated by the opt-in toggle), no new permission (memory `reference_photo_picker_strips_exif_gps`). **versionCode 122→123, versionName 1.1.0-rc7→rc8** — one closed-testing build folding the map + ALL post-vC122 changes (trophy-room polish, weekly-recap-shows-all-finds, chase-the-daily-bird free badge → 34 stamps total = 27 free + 7 premium, weekly trophy notification, splash double-bird fix). Store-listing + What's new updated in repo (EN+SV, v1.0.0→v1.1); data-safety already covers location. Website Vercel Analytics shipped with this PR. **Still manual at AAB upload:** Play Console What's new + store-listing text + data-safety form; a real `MAPTILER_API_KEY` for the release build; fresh v1.1 screenshots/feature-graphic (follow-up #9, still pending). Local `gradle.properties` holds the MapTiler key (never committed).
...
8. **Store-listning + release-notes vid nästa AAB-upload (v1.1, vC123/rc8):** Repo-texten i `docs/play-store/store-listing-{en,sv}.md` är nu **uppdaterad till v1.1** (2026-06-08, commit `0b2ed2a5`): kartan (Premium) + gratis opt-in-plats, omgjorda märken (rödlistat-spår, livslista 500, audio/säsong gratis), ekologiska uppslagsgrupper, märkesantal **34** (27 gratis + 7 premium), och "What's new" v1.0.0→v1.1. **Kvar = bara den manuella Play Console-inmatningen** av samma text + "What's new"-fältet vid upload. (Täcker DP A–E + Phase B + DP B-positionering + post-vC122 + kartan.) Detaljerad batch-state i auto-memory `project_v1_1_release_train.md`.
