...
- **i2a (Android LiteRT + 16 KB + classifier-fix) — SPEC + PLAN KLARA, exekvering startar nästa session (2026-07-16, Windows):** Brainstormad + godkänd design och komplett implementationsplan. Spec: `docs/superpowers/specs/2026-07-16-i2a-android-litert-16kb-design.md`, plan: `docs/superpowers/plans/2026-07-16-i2a-android-litert-16kb.md` (6 tasks, subagent-driven-development rekommenderad; rött-till-grönt-bevis via nytt gate-verktyg `tools/check_16kb_alignment.py`). Nyckelbeslut (Albin): core = `litert:1.4.1` (drop-in, noll källkod), Flex = community-16 KB-bygget `arxdeus/tflite_flex_16kb_android` tag `tf-a95156b81d38` — **binärerna nedladdades, SHA-256-pinnades i planen och ELF-verifierades (p_align=0x4000, alla tre ABI:er) redan under planeringen**; `tensorflow-lite-support` droppas (0 imports); x86 utesluts via abiFilters; classifier-first-run-buggen fixas direkt utan repro-först (buffert-retention i BÅDA runners — audio-runnern har samma latenta defekt — + println-instrumentering i PhotoAnalyzeViewModels tysta getOrElse; detekt-fällor redan lösta i planen: PrintStackTrace aktiv → println, UnusedPrivateProperty aktiv → init-guard). vC126 = versionCode 126 / versionName 1.2.1. Sista grind före Play-upload = Albins device-verify (ren install → foto-ID FÖRSTA försöket, audio node-29, live-scan). **NÄSTA SESSION (Windows): "fortsätt med i2a" → kör planen med SDD.**
...
- **🚀 LIVE I PRODUKTION på Google Play (2026-06-17):** vC125 / 1.2.0-rc3 godkänd + utrullad till produktion (publik: `play.google.com/store/apps/details?id=se.birdy.android` — "Birdy — Bird Identify & Guide"; bekräftad via WebFetch). Hela v1.2 (karta + premium-redesign + UX-polish + scan-freeze-par-modell) ute. Premium öppet/gratis för alla (`PREMIUM_OPEN_FOR_LAUNCH=true`). **Kom förbi 16 KB-page-size-grinden via en per-version-skip i Play Console** (den självbetjänta förlängningen gick ut 2026-05-31; skippen är tillfällig). **KVAR (ej launch-blockerande nu):** (1) **16 KB-fix** — migrera `org.tensorflow:tensorflow-lite` → `com.google.ai.edge.litert` + lös Flex/FlexRFFT-`.so` (BARA de två TFLite-`.so` är 4 KB, uppmätt; AndroidX redan 16 KB) — krävs före NÄSTA update (vC126) + för att appen ska köra på 16 KB-enheter; **beslut 2026-07-07: migrationen görs inne i iOS-spåret som plan i2** (så iOS-ML byggs på nya biblioteket och Android blir release-ready på köpet); se [[project_16kb_page_size_blocker]]. (2) Bevaka launch-data (Android vitals krasch/ANR, installs, recensioner, **MapTiler-kvot**). (3) Billing-verify + **grandfather (tidiga användare behåller premium för alltid)** + AB-transfer (~2–3 v) → flippa monetisering på AB:t. (4) **Classifier-failure-bugg (måste fixas före nästa release, vC126):** foto-vägen visar "Analyzer failed"/`ClassifierFailure` första gången efter ren install (omstart "löser" det); prime suspect = TFLite modell-`ByteBuffer`/`modelBytes` behålls EJ som fält i `AndroidTfliteRunner.kt:42-51` → GC frigör native-minne → dinglande pekare; `PhotoAnalyzeViewModel.kt:34` sväljer felet tyst (ingen logg). Ordning: instrumentera catch → repro i debug → bekräfta → fixa (behåll buffert som fält); se [[project_classifier_failure_first_run_bug]]. Billing-debug-toggle (`c027a6f6`) + grandfather-hard-gate i runbooken redan inlagda; feature-graphic ommatchad till kort-stil (`a3e5a269`). Repo-commits pushade till main.
...
| i2a | Android TFLite→LiteRT (16 KB-fix) | `litert:1.4.1` (fixar foto-`.so`), audio-Flex-`.so`-beslut, regression + 16 KB-verify, vC126-ready | ⬜ Windows-maskinen (Galaxy-verify); oberoende av iOS |
