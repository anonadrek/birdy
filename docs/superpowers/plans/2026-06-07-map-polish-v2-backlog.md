...
- **Item 4 — location for non-live captures: NEXT (real-world bug found 2026-06-08).** Albin saved a real find via **gallery photo (Find)** with location ON → it did NOT appear on the map. Verified root cause (no crash): location attaches only for **live "Look" + "Listen" audio** (`shouldAttachLocation`: Audio→true, Image→`source.live`); gallery + in-app take-photo run through `PhotoAnalyzeScreen` with `live=false` → provider never called. Device location ON + permission granted + `AndroidLocationProvider` robust. See refined design in item 4 below.

### RECOMMENDED NEXT-SESSION PLAN (agreed with Albin 2026-06-08)
1. **Brainstorm + build item 4** (4a + 4b, refined design below) on `feat/map-polish-v2`.
2. **Merge** all of map-polish-v2 (items 1 + 2 + 4) to `main` via PR (like personal-finds-map PR #4).
