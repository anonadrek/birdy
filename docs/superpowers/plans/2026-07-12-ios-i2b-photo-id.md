...
- [x] **Step 1: Download + extract + commit the xcframework (keep only TensorFlowLiteC)**
...
- [x] **Step 2: Write the cinterop def**
...
- [x] **Step 3: Register the cinterop + iosTest deps in the ml build**
...
- [x] **Step 5: Run the smoke test on the simulator**
...
- [x] **Step 6: Link the xcframework in the app project + regenerate**
...
- [x] **Step 1: Write the golden test first**
...
- [x] **Step 4: Run — expect PASS.** `./gradlew :shared:ml:iosSimulatorArm64Test --console=plain` green.

- [x] **Step 5: ktlint + commit** (`git add shared/ml/src/iosMain shared/ml/src/iosTest shared/ml/src/commonTest`; `feat(ios): CoreGraphics ImagePreprocessor + byte-parity test (i2b T2)`).
...
- [x] **Step 1: Replace the classifier bootstrap** — change the `buildClassifier` lambda in `buildIosAppGraph()` to build the real classifier:
...
- [x] **Step 2: Verify iOS links + Android untouched**
...
- [x] **Step 3: Commit** (`feat(ios): wire real photo classifier into iOS AppGraph (i2b T4)`).
...
- [x] **Step 1:** Move the file to commonMain, change the parameter from `android.graphics.Bitmap` to `androidx.compose.ui.graphics.ImageBitmap`, replace `bitmap.asImageBitmap()` internals with the passed `ImageBitmap`, and use `image.width`/`image.height` for geometry. Drop Android-only bitmap-recycle handling (ARC/Compose owns `ImageBitmap`). Keep the pure `fitMapping`/`pickDragMode`/`CropGeometry` math.
- [x] **Step 2:** In `PhotoAnalyzeHost.android.kt`, pass `cropBitmap.asImageBitmap()` to `CropAdjustScreen`; keep rotation done in the host (`rotate90()` on the source `Bitmap`), re-deriving the `ImageBitmap` after rotate.
- [x] **Step 3: Verify Android** — `./gradlew :composeApp:testDebugUnitTest :composeApp:compileDebugKotlinAndroid :androidApp:assembleDebug ktlintCheck detekt` green; `CropGeometryTest` still passes. **Device note for Albin:** re-verify the Android crop still works on the Galaxy at the next Android session (interactive) — flag in commit.
- [x] **Step 4: Verify iOS compiles** — `./gradlew :composeApp:linkDebugFrameworkIosSimulatorArm64` green (crop now visible to iosMain).
- [x] **Step 5: Commit** (`refactor: lift CropAdjustScreen to shared commonMain (ImageBitmap) (i2b T5)`).
...
- [x] **Step 1 (agent):** Build the app (concrete-destination recipe), install to a booted simulator, launch, screenshot. Confirm `:shared:ml:iosSimulatorArm64Test` all green (smoke + preprocessor + parity).
- [x] **Step 2 (agent):** Android shippability gate — `./gradlew :shared:domain:jvmTest :shared:ml:jvmTest :composeApp:testDebugUnitTest :androidApp:assembleDebug ktlintCheck detekt` green.
- [ ] **Step 3 (Albin, interactive) — THE ONLY OPEN ITEM IN i2b:** Drag a bird photo into the simulator's Photos app (or use device); in Birdy, open the photo/scan entry → gallery → pick it → crop → confirm → real classification → correct Match/Disambig screen. Screenshot. Also re-verify Android crop on the Galaxy (from Task 5). **Include a 48 MP HEIC** (iOS has no `inSampleSize` equivalent — the one deferred finding with a crash-shaped failure mode) **and a rapid rotate double-tap** (rotation went async in `782af6db`; two fast taps may net 90° instead of 180°). This is a real gate, not a formality: the ±0.05 numeric-parity guarantee covers preprocessor + runner only, never the composed decode→crop→finalize→analyze path.
- [x] **Step 4 (agent):** Update `CLAUDE.md` (Status + Plan-of-plans i2b row → done/verified), commit, push.
